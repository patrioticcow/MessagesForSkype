...
$(function () {
	...
	if (path !== undefined) {
		var skypeUser = path.replace("C:\\Users\\" + username + "\\AppData\\Roaming\\Skype\\", "");

		$('#skypePath').val(skypeUser);
	...
	$('#saveSkypePathHelp').on('click', function () {
		alert('ex: C:\\Users\\' + username + '\\AppData\\Roaming\\Skype\\[skype user]');
		return false;
	});
	...
	$('#saveSkypePath').on('click', function () {
		var sypeUser = $('#skypePath').val();
		if (sypeUser === '') {
			alert('The path is empty');
			return false;
		}

		var path = 'C:' + s + 'Users' + s + '' + username + '' + s + 'AppData' + s + 'Roaming' + s + 'Skype' + s + sypeUser;
		if (path !== '') {
			var skypeDir = Ti.Filesystem.getFile(path, 'main.db');
...
function clickOnUser(that, response, path) {
	...
	$('#filter').fastLiveFilter('#userInfo');
...
function getSkypeUsername(path) {
	var dirPath     = Ti.Filesystem.getFile(path);
	var dirFiles    = dirPath.getDirectoryListing();
	var dirsToCheck = ['Content', 'DataRv', 'My Skype Received Files', 'shared_dynco', 'shared_httpfe'];

	for (var i = 0; i <= dirFiles.length; i++) {
		if (dirFiles[i] !== undefined && dirFiles[i].isDirectory()) {
			var dir = dirsToCheck.indexOf(dirFiles[i].name());
			if (dir === -1) {
				var skypeUser = dirFiles[i].name();

				var existingPath = getPath();
				if (existingPath === undefined) {
					if (skypeUser !== undefined) insertDefaults(path + skypeUser);
				}
